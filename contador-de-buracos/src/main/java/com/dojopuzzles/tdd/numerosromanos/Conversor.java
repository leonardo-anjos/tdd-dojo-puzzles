package com.dojopuzzles.tdd.numerosromanos;
...
public class Conversor {
	...
	public int converteAlgarismoRomanoParaArabico(String algarismoRomano) throws Exception {
		int resultado = 0;
		Character caractere;
		...
		if (algarismoRomano.length() == 1) {
			...
			for (int j = 0; j < algarismoRomano.length() - 1; j++) {
				...
				if (conjunto.get(caractere) < conjunto.get(algarismoRomano.charAt(j + 1))) {

					resultado += conjunto.get(algarismoRomano.charAt(j + 1)) - conjunto.get(caractere);
				}
				if (conjunto.get(caractere) == conjunto.get(algarismoRomano.charAt(j + 1))) {
